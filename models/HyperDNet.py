...
class _HyperDLayer(nn.Sequential):
...
class HyperDNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.top3 = _HyperDLayer(num_init_features*3,base_out)
        self.top4 = _HyperDLayer(num_init_features*4,base_out*2)
        ...
        self.bottom3 = _HyperDLayer(num_init_features*3,base_out)
        self.bottom4 = _HyperDLayer(num_init_features*4,base_out*2)
        self.bottom5 = _HyperDLayer(num_init_features*6, base_out*2)
        # self.final_conv = Final_conv(base_out*4,base_out*4)
    ...
    def forward(self,x):
        ...
        # 2 192 64 32 32
        y2_t_i = torch.cat([y1_t_i,y1_t_o,y1_b_o],dim=1)
        y2_b_i = torch.cat([y1_b_i,y1_b_o,y1_t_o],dim=1)
        ...
        # third cat   256
        y3_t_i = torch.cat([y2_t_i, y2_t_o, y2_b_o],dim=1)
        y3_b_i = torch.cat([y2_b_i, y2_b_o, y2_t_o],dim=1)
        # 64
        ...
        y4 = torch.cat([y4_t_o,y4_b_o],dim=1)
